...
const Login = () => {
    ...
    if (!Toggle) {
      createUserWithEmailAndPassword(
        ...
        .catch((error) => {
          // ..
          setError(
            "UserId already in use,Please registered using some other Id"
          );
        });
    } else {
      signInWithEmailAndPassword(
  ...
  return (
    <div>
      ...
      />
      <form
        className="h-auto  md:w-3/12 w-9/12 bg-black m-36 mx-auto right-0 left-0 bg-opacity-75 rounded-xl absolute"
      ...
      >
        ...
        <button
          type="submit"
          className="mx-6 my-3 p-2 bg-blue-700 rounded-lg"
          onClick={handleCheckValidation}
        >
          Submit
        </button>
        <p className="text-red-600 px-6">{Error}</p>
        <p className="p-4 text-green-700" onClick={handleClick}>
          {Toggle
            ? "Are you new,Let's Sign Up"
